...
export function Card({ event }: CardProps) {
  return (
    <article className='bg-white rounded-md p-2'>
      <div className='flex justify-between'>
        <h2 className='text-xl font-bold'><Link href={'/events/' + event.id}>{event.name}</Link></h2>
        <h2 className='text-slate-400'>{event.league && (event.league as League).name}</h2>
      </div>
      <ul className='py-4'>
