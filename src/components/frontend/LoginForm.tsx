...
export default function LoginForm() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (resData.errors) {
      setError('root', {
        message: resData.errors.map((errObj: { message: string }) => (errObj.message + "\n"))
      });
      return;
    }

    setError('root', { message: "Something went wrong on our end." });
  }
  ...
  return (
    <form className="grid gap-6" onSubmit={handleSubmit(onSubmit)}>
      <ErrorMessage error={errors.root as any} />
      <div>
